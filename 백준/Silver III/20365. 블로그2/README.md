...
### 아하!
- 문자열에서 연속된 문자의 개수 세기 -> before와 current가 달라지면 개수++
- 연속된 B와 R의 개수를 각각 세고, 더 작은 개수의 색깔에 1을 더한다

### 느낀점
- 처음엔 풀이를 전혀 모르겠지만,
- 예시 하나로 출력이 나오는 과정을 따라가다보면
- 방법이 어렴풋이 떠오른다.
- 그 방법을 말로 바꿔본다. 그럼 모순이나 이상한 점이 생기는데
- 계속 보완해본다.
- 예시 -> 방법 캐치 -> 구체화

### 성능 요약

메모리: 25748 KB, 시간: 448 ms
