...
### 아하!
#### 1. 자료형 범위
- int형 = 32bit 이므로-2^31 ~ (2^31 - 1)
- long형 = 64bit 이므로 -2^63 ~ (2^63 - 1)
#### 2. List 정렬
...
- 홀수와 짝수의 경우로 나눈다.
- 짝수라면
- 리스트 정렬 후, 작은 값과 큰 값을 더해간다
- 1 10 15 17 이 있으면, 1+18 = 19, 10+15=25, 그럼 M은 25가 된다.
- 홀수라면
- 마지막 원소는 단독으로 포함해서 비교한다. 1 10 15 17 27 이라면 M은 27이 된다.

### 성능 요약

메모리: 32588 KB, 시간: 536 ms
